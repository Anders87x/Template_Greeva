...
function($) {
    ...
    $("#toastr-one").on('click', function (e) {
        $.NotificationApp.send("Heads up!", "This alert needs your attention, but it is not super important.", 'top-left', '#3b98b5', 'info');
    });

    $("#toastr-two").on('click', function (e) {
        $.NotificationApp.send("Heads up!", "Check below fields please.", 'top-right', '#da8609', 'warning');
    });

    $("#toastr-three").on('click', function (e) {
        $.NotificationApp.send("Well Done!", "You successfully read this important alert message", 'top-center', '#5ba035', 'success');
    });

    $("#toastr-four").on('click', function (e) {
        $.NotificationApp.send("Oh snap!", "Change a few things up and try submitting again.", 'bottom-left', '#bf441d', 'error');
    });
    ...
    $("#toastr-six").on('click', function (e) {
        ...
        'bottom-center', '#1ea69a', 'info', false);
    });

    $("#toastr-seven").on('click', function (e) {
        $.NotificationApp.send("", "Set the `hideAfter` property to false and the toast will become sticky.", 'top-left', '#1ea69a', '');
    });
    ...
    $("#toastr-eight").on('click', function (e) {
        ...
        'top-left', '#1ea69a', 'info', 3000, 1, 'fade');
    });
    ...
    $("#toastr-nine").on('click', function (e) {
        $.NotificationApp.send("Slide transition", "Set the `showHideTransition` property to fade|plain|slide to achieve different transitions.",
             'top-left', '#1ea69a', 'info', 3000, 1, 'slide');
    });
    ...
    $("#toastr-ten").on('click', function (e) {
        $.NotificationApp.send("Plain transition", "Set the `showHideTransition` property to fade|plain|slide to achieve different transitions.",
         'top-left', '#3b98b5', 'info', 3000, 1, 'plain');
    });
}(window.jQuery);